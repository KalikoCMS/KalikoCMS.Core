...
namespace KalikoCMS.Data.Maps {
    ...
    internal class PropertyMap : MappingConfiguration<PropertyEntity> {
        internal PropertyMap() {
            MapType(x => new { }).WithConcurencyControl(OptimisticConcurrencyControlStrategy.Changed).ToTable("Property");
            ...
            HasAssociation(x => x.PageProperties).WithOpposite(x => x.Property).ToColumn("PropertyId").HasConstraint((y, x) => x.PropertyId == y.PropertyId);
