...
(function($) {
    ...
    var pluginName = "WYSIWYG",
        defaults = {
            toolbarClass: 'html-toolbar',
            oddColor: '#eee'
        },
        instanceCounter = 0;
    
    $.fn.editHtml = function(options) {
    ...
    function Plugin(element, options) {
        ...
        function addFormatDropdown(toolbar, context) {
            ...
            addFormatOption($options, 'Header 1', 'h1', 'font-size:1.6em;');
            addFormatOption($options, 'Header 2', 'h2', 'font-size:1.4em;');
            addFormatOption($options, 'Header 3', 'h3', 'font-size:1.2em;');
            addFormatOption($options, 'Quote', 'blockquote');
            
            toolbar.append($dropdown);
        }

        function addFormatOption(optionList, label, format, style) {
            var $option = $('<li><a href="javascript:;" unselectable="on" style="' + style + '">' + label + '</a></li>');
            $('a', $option).click(function() { commands.applyFormat(format); });
        ...
        function addToolInGroup(toolbar, icon, command, parameter) {
            var $icon = $('<a href="javascript:;" class="btn" unselectable="on"><i class="' + icon + '"></i></a>');
            $icon.click(function() { command(parameter); });
            toolbar.append($icon);
        }

        function applyCommand(command, param) {
            contentWindow.focus();
        ...
        function insertHtml(html) {
            ...
            if (sel.rangeCount) {
                var xrange = sel.getRangeAt(0);

                var node = $(html)[0];
                xrange.insertNode(node);

                rangy.getSelection().setSingleRange(xrange);
            }
...
/*            
            ...
            var splitNode = function(node, offset) { 
                if (node.nodeType == 3 && offset > 0 && offset < node.length) {
                    rangy.dom.splitDataNode(node, offset);
                }
            };
            
            splitNode(selection.anchorNode, selection.anchorOffset);
            splitNode(selection.focusNode, selection.focusOffset);
            var parent = findParentNode("DIV|H1|H2|H3|H4|H5|H6|P|PRE|BLOCKQUOTE|ADDRESS|FONT|SPAN");
            rangy.CssClassApplier.util.splitNodeAt(parent, selection.anchorNode, selection.anchorOffset); 
            rangy.CssClassApplier.util.splitNodeAt(parent, selection.focusNode, selection.focusOffset);
...
*/
            ...
            if (isCollapsed) {
                ...
                newNode.appendChild(contentDocument.createTextNode(content.textContent));
...
/**/            
        ...
        function findParentNode(types) {
            var typeArray = types.split("|");
            var node = getCurrentNode();
            
            while(node) {
                if($.inArray(node.nodeName, typeArray) > -1) {
                    return node;
                }
